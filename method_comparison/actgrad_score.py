...
if __name__ == '__main__':
	...
	if not os.path.exists(args.out_root+'/'+config.name+'/'+imageset+'/actxgrad'):
		os.makedirs(args.out_root+'/'+config.name+'/'+imageset+'/actxgrad',exist_ok=True)
	torch.save(save_object,'%s/%s/%s/actxgrad/%s_%s:%s.pt'%(args.out_root,config.name,imageset,config.name,layer,str(unit)))
